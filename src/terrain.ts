...
let terrain = {
    read: (room:Room):Room=>{
        let lastTick:number = Memory.cpu.lastTickTime[0][_.size(Memory.cpu.lastTickTime[0])-1]
        let tickBefireLastTick:number = Memory.cpu.lastTickTime[0][_.size(Memory.cpu.lastTickTime[0])-2]
        if(lastTick >= Game.cpu.limit || tickBefireLastTick >= Game.cpu.limit){
            ...
            if(Game.cpu.bucket > settings.getGeneralSettings().bucketLimit && lastTick < Game.cpu.limit){
                let iteration:number = Memory.terrainX
                ...
                for (let i:number = 0; i < settings.getGeneralSettings().roomLength; i++) {
                    data = [].concat(data, {
                        x: iteration,
                        y: i,
