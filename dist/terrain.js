...
var terrain = {
    read: function read(room) {
        var lastTick = Memory.cpu.lastTickTime[0][_.size(Memory.cpu.lastTickTime[0]) - 1];
        var tickBefireLastTick = Memory.cpu.lastTickTime[0][_.size(Memory.cpu.lastTickTime[0]) - 2];
        if (lastTick >= Game.cpu.limit || tickBefireLastTick >= Game.cpu.limit) {
            ...
            if (Game.cpu.bucket > settings.getGeneralSettings().bucketLimit && lastTick < Game.cpu.limit) {
                var iteration = Memory.terrainX;
                if (iteration === settings.getGeneralSettings().roomLength) {
                    Memory.terrainX = 0;
                    iteration = 0;
                }
                var data = [];
                for (var i = 0; i < settings.getGeneralSettings().roomLength; i++) {
                    data = [].concat(data, {
                        x: iteration,
                        y: i,
